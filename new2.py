        ...
        if "table" in user_question.lower():
            ...
            for table in tables:
                if table:  # Check if the table is not empty
                    ...
                    st.write(f"Table from {pdf_name}:")
                    st.table(df)
...
def main():
    ...
    if user_question:
        ...
        tables_dict = st.session_state.get("tables_dict", {})
        user_input(user_question, text_dict, tables_dict)
    ...
    with st.sidebar:
        ...
        if st.button("Submit & Process"):
            with st.spinner("Processing..."):
                text_dict, tables_dict = get_pdf_text_and_tables(pdf_docs)
                st.session_state["text_dict"] = text_dict
                st.session_state["tables_dict"] = tables_dict
